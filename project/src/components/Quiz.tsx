...
export function Quiz({ onComplete }: QuizProps) {
  ...
  const handleAnswer = async (selectedOption: number) => {
    const correct = selectedOption === questions[state.currentQuestion].correctAnswer;
  ...
  if (state.isComplete) {
    return (
      <div className="text-center p-8 bg-white rounded-lg shadow-lg">
        ...
        <p className="text-xl">Your score: {state.score} / {questions.length}</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white p-8 rounded-lg shadow-lg max-w-2xl w-full">
      <div className="mb-6">
        <p className="text-sm text-pink-600 mb-2">Question {state.currentQuestion + 1} of {questions.length}</p>
